...
resource "aws_api_gateway_method" "post_action" {
  rest_api_id = "${aws_api_gateway_rest_api.api.id}"
...
resource "aws_api_gateway_integration" "post_action" {
  rest_api_id = "${aws_api_gateway_rest_api.api.id}"
  resource_id = "${aws_api_gateway_resource.action.id}"
  http_method = "${aws_api_gateway_method.post_action.http_method}"
  type = "AWS_PROXY"
  uri = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.aws_region}:${data.aws_caller_identity.current.account_id}:function:${aws_lambda_function.action.function_name}/invocations"
  integration_http_method = "POST"
}
